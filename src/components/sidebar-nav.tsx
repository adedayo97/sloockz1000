...
type SidebarNavProps = {
    ...
    currentWorkspaceData: Workspace | null;  // Allow `currentWorkspaceData` to be null
};
                    ...
                    <div className="cursor-pointer items-center text-white mb-4 w-10 h-10 rounded-lg overflow-hidden">
                        <Popover>
                            <PopoverTrigger>
                                <Avatar>
                                    <AvatarImage 
                                        src={currentWorkspaceData?.image_url || ""}  // Safe access
                                        alt={currentWorkspaceData?.name || "Workspace"}  // Safe access
                                        className="object-cover w-full h-full"
                                    />
                                    <AvatarFallback className="bg-neutral-700">
                                        <Typography  
                                            variant="p"
                                            text={currentWorkspaceData?.name?.slice(0, 2) || "WS"}  // Safe access
                                        />
                                    </AvatarFallback>
                                </Avatar>
                            </PopoverTrigger>
                                    ...
                                    <CardContent className="flex p-0 flex-col">
                                        {switchingWorkspace ? (
                                            ...
                                            userWorkspaceData.map((workspace) => {
                                                const isActive = workspace.id === currentWorkspaceData?.id;  // Safe access
                                                return (
                                                    <div 
                                                        key={workspace.id} 
                                                        className={cn(
